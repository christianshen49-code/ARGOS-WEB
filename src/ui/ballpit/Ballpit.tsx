...
/**
 * Ballpit — React Bits component port to TS.
 *
 * Vendor code: minified-style single-letter symbols, private fields, custom
 * shader subclass. Retyping it adds zero runtime value, so the file is
 * `@ts-nocheck`. The exported React component itself has a proper props type.
 *
 * Original credit: Kevin Levron (https://x.com/soju22/status/1858925191671271801)
 */
...
function createBallpit(canvas, t = {}) {
  ...
  let spheres;
  inst.renderer.toneMapping = v;
  inst.camera.position.set(0, 0, 20);
  inst.camera.lookAt(0, 0, 0);
  inst.cameraMaxAspect = 1.5;
  inst.resize();
  initialize(t);
  const ray = new y();
  ...
  const ptr = S({
    domElement: canvas,
    onMove() {
      ray.setFromCamera(ptr.nPosition, inst.camera);
      inst.camera.getWorldDirection(plane.normal);
  ...
  function initialize(e) {
    if (spheres) {
      inst.clear();
      inst.scene.remove(spheres);
    }
    spheres = new Z(inst.renderer, e);
    inst.scene.add(spheres);
  }

  inst.onBeforeRender = (e) => {
    if (!paused) spheres.update(e);
  };
  inst.onAfterResize = (e) => {
  ...
  return {
    ...
    setCount(e) {
      initialize({ ...spheres.config, count: e });
    },
    togglePause() {
      paused = !paused;
    },
...
const Ballpit = ({ className = "", style, followCursor = true, ...props }: BallpitProps) => {
